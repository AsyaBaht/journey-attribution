...
INSIGHTS: list[tuple[str | None, str]] = [
    ...
    (
        ...
        "rewarding frequency, not causal contribution. That suspicion is no longer "
        "just a suspicion: on simulated data with known ground truth, the Markov "
        "removal effect's channel ranking is a <b>perfect rank-copy of touchpoint "
        "volume (Spearman ρ = 1.000)</b> and scores worst of all seven methods "
        "against the true counterfactual — so read the Markov column here as a "
        "volume ranking. The data-driven (Shapley) "
